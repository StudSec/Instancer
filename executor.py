...
def runner(server, cmd) -> tuple[Server, str | None]:
    try:
...
class Executor:
    ...
    async def run_all(self, cmd) -> list[tuple[Server, str]]:
        result = await asyncio.gather(
            *[asyncio.to_thread(runner, server, cmd) for server in self.config.servers]
        )
        return [(server, response) for (server, response) in result if response != None]

    async def run(self, server, cmd) -> str | None:
        _, result = await asyncio.to_thread(runner, server, cmd)
        return result
